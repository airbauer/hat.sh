const ja_JP = {
  ...
  changelog: '変更履歴',
  donation_message:
    'hat.shを気に入ってくれましたか? 寄付をすることでこのプロジェクトを支援できます。',
  ...
  help_translate:
    'あなたの言語が見つかりませんでしたか? githubの翻訳ガイドにしたがって、このアプリの翻訳をサポートすることができます。',
  reload: '再読み込み',
  dark_mode: 'ダークモード',
  close: '閉じる',
  reset: 'リセット',
  ...
  multiple_tabs_alert_notice_two:
    'ファイルの暗号化や復号化を複数のタブで同時に行わないでください。',
  understand: 'わかりました',
  ...
  offline_note:
    'ファイルはサーバーにアップロードされず、すべてはあなたのブラウザ上でオフラインで処理されます。',
  ...
  copy_link: 'リンクをコピー',
  after_enc_note_one:
    '受信者が復号化するには、このファイルをあなたの公開鍵とともに共有する必要があります。',
  after_enc_note_two:
    '公開鍵を含むリンクを作成することで、公開鍵を伝えたり、受信者が入力する必要がありません。',
  ...
  file_not_encrypted_corrupted:
    'このファイルはhat.shを使用して暗号化されていないか、ファイルが破損している可能性があります！',
  old_version:
    'このファイルは古いバージョンのhat.shを使用して暗号化されています。v1アプリにアクセスして復号化することができます。',
  file_mixup:
    '選択されたファイルを復号化するには、暗号化時のパスワードもしくは公開鍵どちらか同じ方法を使用する必要があります。ファイルに適合する方法を選択してください。',
  ...
  file_has_wrong_password:
    'はパスワードが違います。パスワードテストを停止しました。すべてのファイルが同様の正しい復号化パスワードであることを確認してください。',
  file_has_wrong_keys:
    'は鍵が間違っています。鍵のテストを停止しました。すべてのファイルが同様の正しい復号化鍵であることを確認してください。',
  ...
  limited_private:
    'プライベートブラウジングのため機能が限定されています (1つのファイルのみ、1GBまで)',
  ...
  qr_code_note_one:
    'このQRコードにはあなたの公開鍵が含まれます。スキャン後、アプリにリダイレクトされ、公開鍵が自動入力されます。',
  qr_code_note_two:
    '暗号化の際、手動であなたの公開鍵を入力する代わりに、このQRコードまたはリンクを他人と共有することができます。',
