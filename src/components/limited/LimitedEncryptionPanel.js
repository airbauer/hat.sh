...
const LimitedEncryptionPanel = () => {
    ...
    try {
      ...
      if (key) {
        ...
        if (mode === 'derive' && encRx && encTx) {
          let limitedRes = sodium.crypto_secretstream_xchacha20poly1305_init_push(encTx);
  ...
  const startLimitedEncryption = (file) => {
    ...
    if (encryptionMethod === 'publicKey') {
      const SIGNATURE = new Uint8Array(encoder.encode(SIGNATURES['v2_asymmetric']));
  ...
  const limitedChunkEncryption = async (limitedLast, chunk, file) => {
    ...
    const limitedEncryptedChunk = sodium.crypto_secretstream_xchacha20poly1305_push(
      limitedState,
      ...
      <Stepper activeStep={activeStep} orientation="vertical" className={classes.stepper}>
        <Step key={1}>
          ...
          <StepContent>
            ...
            {!largeFile && <Typography className={classes.offline}>{t('offline_note')}</Typography>}
          </StepContent>
        </Step>
        ...
        <Step key={2}>
          ...
          >
            {encryptionMethod === 'secretKey' ? t('enter_password_enc') : t('enter_keys_enc')}
          </StepLabel>
          <StepContent>
            <FormControl component="fieldset" style={{ float: 'right', marginBottom: '15px' }}>
              <RadioGroup row defaultValue={encryptionMethod} aria-label="encryption options">
                <FormControlLabel
                  value="secretKey"
                  ...
                  <Grid item xs>
                    <Button
                      disabled={
                        ...
                        (encryptionMethod === 'publicKey' && (!PublicKey || !PrivateKey))
                      }
                      variant="contained"
              ...
              <Grid container spacing={1}>
                ...
                <Grid item xs>
                  <Button
                    ...
                    disabled={isEncrypting || (!Password && !PublicKey && !PrivateKey) || !File}
                    variant="contained"
                    className={`${classes.nextButton} nextBtnHs`}
                    startIcon={
                      isEncrypting ? (
      ...
      {activeStep === 3 && (
        <Paper elevation={1} className={classes.resetContainer}>
          ...
          <Grid container spacing={1}>
            ...
            {encryptionMethod === 'publicKey' && (
              <Grid item xs={12} sm={6}>
                <Tooltip title="Create a link that has your public key" placement="bottom">
                  <Button
            ...
            {encryptionMethod === 'publicKey' && shareableLink && (
              <TextField
                style={{ marginTop: 15 }}
                defaultValue={shareableLink != undefined ? shareableLink : shareableLink}
                InputProps={{
                  readOnly: true,
                  classes: {
